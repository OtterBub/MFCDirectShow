...
#include "pch.h"
#include "SKP-arkFilter_Guids.h""
#include "SKP-arkFilter.h"
...
{
	&CLSID_SKParkFilter,                // Filter CLSID
	L"SKParkFilter",                    // String name
...
CFactoryTemplate g_Templates[] = {
	L"SKParkFilter", &CLSID_SKParkFilter, CSKPark::CreateInstance, NULL, &sudDump
};
int g_cTemplates = 1;
...
CSKParkFilter::CSKParkFilter(CSKPark *pDump,
	LPUNKNOWN pUnk,
	CCritSec *pLock,
	HRESULT *phr) :
	CBaseFilter(NAME("SKParkFilter"), pUnk, pLock, CLSID_SKParkFilter),
	m_pDump(pDump)
{
}
...
// GetPin
//
CBasePin * CSKParkFilter::GetPin(int n)
{
	if (n == 0) {
...
//
STDMETHODIMP CSKParkFilter::Stop()
{
...
//
STDMETHODIMP CSKParkFilter::Pause()
{
	CAutoLock cObjectLock(m_pLock);
...
// Overriden to open the dump file
//
STDMETHODIMP CSKParkFilter::Run(REFERENCE_TIME tStart)
{
	CAutoLock cObjectLock(m_pLock);
...
//
CSKParkInputPin::CSKParkInputPin(CSKPark *pDump,
	LPUNKNOWN pUnk,
	CBaseFilter *pFilter,
	CCritSec *pLock,
	...
	CRenderedInputPin(NAME("CSKParkInputPin"),
		pFilter,                   // Filter
		pLock,                     // Locking
		phr,                       // Return code
		L"Input"),                 // Pin name
...
//
HRESULT CSKParkInputPin::CheckMediaType(const CMediaType *)
{
	return S_OK;
}
...
// Break a connection
//
HRESULT CSKParkInputPin::BreakConnect()
{
...
//
STDMETHODIMP CSKParkInputPin::ReceiveCanBlock()
{
	return S_FALSE;
}
...
//
STDMETHODIMP CSKParkInputPin::Receive(IMediaSample *pSample)
{
...
//
HRESULT CSKParkInputPin::WriteStringInfo(IMediaSample *pSample)
{
	CheckPointer(pSample, E_POINTER);
...
// EndOfStream
//
STDMETHODIMP CSKParkInputPin::EndOfStream(void)
{
	CAutoLock lock(m_pReceiveLock);
...
//
STDMETHODIMP CSKParkInputPin::NewSegment(REFERENCE_TIME tStart,
...
//  CDump class
//
CSKPark::CSKPark(LPUNKNOWN pUnk, HRESULT *phr) :
	CUnknown(NAME("CSKPark"), pUnk),
	m_pFilter(NULL),
	m_pPin(NULL),
	m_pPosition(NULL),
	...
	m_fWriteError(0)
{
	...
	m_pFilter = new CSKParkFilter(this, GetOwner(), &m_Lock, phr);
	if (m_pFilter == NULL) {
		if (phr)
			*phr = E_OUTOFMEMORY;
		return;
	...
	m_pPin = new CSKParkInputPin(this, GetOwner(),
		m_pFilter,
		&m_Lock,
		&m_ReceiveLock,
		phr);
...
//
STDMETHODIMP CSKPark::SetFileName(LPCOLESTR pszFileName, const AM_MEDIA_TYPE *pmt)
{
...
// Implemented for IFileSinkFilter support
//
STDMETHODIMP CSKPark::GetCurFile(LPOLESTR * ppszFileName, AM_MEDIA_TYPE *pmt)
{
...
//
CUnknown * WINAPI CSKPark::CreateInstance(LPUNKNOWN punk, HRESULT *phr)
{
	ASSERT(phr);

	CSKPark *pNewObject = new CSKPark(punk, phr);
	if (pNewObject == NULL) {
		if (phr)
			*phr = E_OUTOFMEMORY;
	}
...
//
STDMETHODIMP CSKPark::NonDelegatingQueryInterface(REFIID riid, void ** ppv)
{
...
//
HRESULT CSKPark::Write(PBYTE pbData, LONG lDataLength)
{
	DWORD dwWritten;
...
HRESULT CSKPark::HandleWriteFailure(void)
{
	DWORD dwErr = GetLastError();
...
//
void CSKPark::WriteString(TCHAR *pString)
{
	ASSERT(pString);
